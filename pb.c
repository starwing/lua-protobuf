...
static int lpb_addtype(lua_State *L, pb_Buffer *b, int idx, int type, int ignore_zero) {
    lpb_Value v;
    size_t len = 0;
    int ret = 1;
    switch (type) {
    case PB_Tbool:
        ...
        if (v.u32 == 0 && ignore_zero) return -1;
        len = pb_addvarint32(b, v.u32);
        break;
    case PB_Tdouble:
        v.lnum = lua_tonumberx(L, idx, &ret);
        if (ret && ignore_zero && v.lnum == 0.0) return -1;
    ...
    case PB_Tfixed32:
        ...
        if (ret && ignore_zero && v.u64 == 0) return -1;
    ...
    case PB_Tbytes: case PB_Tstring:
        *v.s = lpb_toslice(L, idx);
        ret = (v.s->p != NULL);
        if (ret && ignore_zero && pb_len(*v.s) == 0) return -1;
        if (ret) len = pb_addbytes(b, *v.s);
        break;
    default:
...
static int lpbE_enum(lpb_Env *e, const pb_Field *f, int idx, int ignore_zero) {
    lua_State *L = e->L;
    pb_Buffer *b = e->b;
    const pb_Field *ev;
    int type = lua_type(L, idx);
    if (type == LUA_TNUMBER) {
        ...
        if (ignore_zero && v == 0) return -1;
        return (int)lpb_checkmem(L, pb_addvarint64(b, v));
    } else if ((ev = pb_fname(f->type,
                    lpb_name(e->LS, lpb_toslice(L, idx)))) != NULL) {
        if (ignore_zero && ev->number == 0) return -1;
        return (int)lpb_checkmem(L, pb_addvarint32(b, ev->number));
...
static int lpbE_field(lpb_Env *e, const pb_Field *f, int idx, int ignore_zero) {
    lua_State *L = e->L;
    pb_Buffer *b = e->b;
    size_t len;
    int r;
    ...
    case PB_Tenum:
        ...
        return lpbE_enum(e, f, idx, ignore_zero);
    case PB_Tmessage:
        if (e->LS->use_enc_hooks) lpb_useenchooks(L, e->LS, f->type);
        ...
        if (ignore_zero && len == pb_bufflen(b)) return -1;
        return (int)lpb_addlength(L, b, len, 1);
    default:
        r = lpb_addtype(L, b, idx, f->type_id, ignore_zero);
        argcheck(L, r != 0, 2, "%s expected for field '%s', got %s",
...
static void lpbE_tagfield(lpb_Env *e, const pb_Field *f, int idx, int ignore_zero) {
    size_t oldlen = pb_bufflen(e->b);
    int len;
    if (e->LS->encode_default_values) ignore_zero = 0;
    lpb_checkmem(e->L, pb_addvarint32(e->b,
            pb_pair(f->number, pb_wtypebytype(f->type_id))));
    len = lpbE_field(e, f, idx, ignore_zero);
    if (len == -1) pb_bufflen(e->b) = oldlen;
}
...
static void lpb_encode_onefield(lpb_Env *e, const pb_Type *t, const pb_Field *f, int idx) {
    if (f->type && f->type->is_map)
        ...
        lpbE_tagfield(e, f, idx, t->is_proto3 && !f->oneof_idx && f->type_id != PB_Tmessage);
}
...
static int lpbE_pack(lpb_Env* e, const pb_Type* t, int idx) {
    ...
    lpb_checkmem(L, f != NULL);
    for (i = 0; i < t->field_count; i++) {
        int cur = idx + i;
        if (!lua_isnoneornil(L, cur)) {
            lpb_encode_onefield(e, t, f[i], cur);
        }
    }
    return 0;
}
...
static int Lpb_pack(lua_State* L) {
    ...
    if (e.b == NULL)
        idx = 2, pb_resetbuffer(e.b = &LS->buffer);
    lpbE_pack(&e, t, idx);
    if (e.b != &LS->buffer)
        lua_settop(L, 3);
    else {
