...
For converting between binary protobuf data with Lua tables, using `pb.load()`/`pb.loadfile()` loads the compiled protobuf schema file (`*.pb`) generated by Google protobuf's compiler named `protoc` and call `pb.encode()`/`pb.decode()`.
...
If you don't want to depend Google's protobuf compiler, `protoc.lua` is a pure Lua module translating text-based protobuf schema file into the `*.pb` binary format.
